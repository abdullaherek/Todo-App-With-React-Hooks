...
export default function App() {
  const data = {
    foundation: {
      steps: [
        { key: "1", title: "setup virtual office", isDone: false, disabled:false },
        { key: "2", title: "set mission and vision", isDone: false , disabled:false },
        { key: "3", title: "select business name", isDone: false , disabled:false },
    ...
    discovery: {
      steps: [
        ...
        { key: "2", title: "competitor analysis", isDone: false , disabled:true},
      ],
    },

    delivery: {
      steps: [
        { key: "1", title: "release marketing website", isDone: false, disabled:true },
  ...
  useEffect(() => {
    const filteredTodo = todoList.foundation.steps.filter((item)=> item.isDone);
 ...
 useEffect(()=> {
   const filteredTodoDiscovery = todoList.discovery.steps.filter((item)=> item.isDone);
  ...
  const updateCheckDiscovery = ({ isDone, key }) => {
    setTodoList(state => ({
      ...
      discovery: {
        ...
        steps: state.discovery.steps.map(todo => todo.key === key
          ? { ...todo, isDone }
          : todo)
      },
    }));
  ...
  const updateCheckDelivery = ({ isDone, key }) => {
    setTodoList(state => ({
      ...
      delivery: {
        ...
        steps: state.delivery.steps.map(todo => todo.key === key
          ? { ...todo, isDone }
          : todo)
      },
    }));
